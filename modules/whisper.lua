...
function mod:CHAT_MSG_WHISPER(event_name, msg, sender)
  ...
  local eligible = EPGP:GetEPGP(member)
  if not eligible and not string.match(member,"-") then
    member = member .. "-" .. GetRealmName()
    eligible = EPGP:GetEPGP(member)
  end
...
function mod:CHAT_MSG_BN_WHISPER(event_name, msg, sender,_,_,_,_,_,_,_,_,_,_,userID)
  if not UnitInRaid("player") then return end
  ...
  if not eligible then
    BNSendWhisper(userID, L["%s is not eligible for EP awards"]:format(member))
  elseif EPGP:IsMemberInAwardList(member) then
    BNSendWhisper(userID, L["%s is already in the award list"]:format(member))
  else
    EPGP:SelectMember(member)
    BNSendWhisper(userID,L["%s is added to the award list"]:format(member))
  end
end
...
local function SendNotifiesAndClearExtras(
  ...
  if extras_awarded then
    for member,_ in pairs(extras_awarded) do
      ...
      sender = senderBNMap[member]
      if sender then
        BNSendWhisper(sender,L["%+d EP (%s) to %s"]:format(
                          extras_amount, extras_reason, member))
        EPGP:DeSelectMember(member)
        BNSendWhisper(
          sender,L["%s is now removed from the award list"]:format(member))
      end
      senderBNMap[member] = nil
